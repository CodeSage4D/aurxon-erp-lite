...
import { ProvisioningService } from '../Licensing/provisioning.service';
...
export class RegistrationService {
  ...
  async create(dto: CreateRegistrationDto) {
    ...
    const phoneExists = await this.prisma.organizationRegistration.findFirst({
    ...
    if (dto.gstNumber) {
      const gstUpper = dto.gstNumber.trim().toUpperCase();
      const gstExists = await this.prisma.organizationRegistration.findFirst({
        where: { gstNumber: gstUpper },
      });
      if (gstExists) {
    ...
    const orgExists = await this.prisma.organizationRegistration.findFirst({
      where: { orgName: { equals: dto.orgName.trim(), mode: 'insensitive' } },
    });
    if (orgExists) {
      throw new BadRequestException('Organization Name is already registered.');
    }
